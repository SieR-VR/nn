...
class NNParser(Parser):
    ...
    def source(self) -> Optional[Source]:
        ...
        if (stmts := self.statements(),) and (self.expect("ENDMARKER")):
    ...
    def statement(self) -> Optional[Statement]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (stmt := self.simple_stmt()) and (self.expect(";"),) and (self._loop1_2(),):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return Statement(
                stmt,
        ...
        self._reset(mark)
        if (stmt := self.compound_stmt()) and (self._loop1_3(),):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return Statement(
                stmt,
    ...
    def simple_stmt(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (self.positive_lookahead(self.expect, "let")) and (
            decl_stmt := self.decl_stmt()
        ):
            return decl_stmt
        self._reset(mark)
        if (self.positive_lookahead(self.expect, "import")) and (
    ...
    def compound_stmt(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if func_decl_stmt := self.func_decl_stmt():
            return func_decl_stmt
        self._reset(mark)
        if expr_stmt := self.expr_stmt():
            return expr_stmt
        self._reset(mark)
        if (self.positive_lookahead(self.expect, "export")) and (
    ...
    def func_decl_stmt(self) -> Optional[FuncStatement]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (name := self.name()) and (func_expr := self.func_expr()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
    ...
    def import_stmt(self) -> Optional[ImportStatement]:
        ...
        if (
            (self.expect("import"))
            and (import_list := self.import_list())
            and (self.expect("from"))
            and (import_dest := self.import_dest())
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
    ...
    def export_stmt(self) -> Optional[ExportStatement]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (self.expect("export")) and (expr := self.expr()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
    ...
    def import_item(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (name := self.name()) and (opt := self._tmp_5(),):
            return [name, opt]
        self._reset(mark)
        return None
    ...
    def block_stmt(self) -> Optional[Any]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (self.expect("{")) and (stmts := self.statements(),) and (self.expect("}")):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
    ...
    def expr(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if func_expr := self.func_expr():
            return func_expr
        self._reset(mark)
        if call_expr := self.call_expr():
            return call_expr
        self._reset(mark)
        if ident_expr := self.ident_expr():
            return ident_expr
        self._reset(mark)
        return None
    ...
    def func_expr(self) -> Optional[FuncExpression]:
        ...
        if (
            ...
            and (type_expr_list := self.type_expr_list(),)
            and (self.expect("("))
            and (arg_decl_a := self.arg_decl())
            and (arg_decl_b := self._loop0_6(),)
            and (self.expect(","),)
            and (self.expect(")"))
            and (self.expect(":"))
            and (type_expr := self.type_expr())
            and (self._loop0_7(),)
            and (block_stmt := self.block_stmt())
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
    ...
    def call_expr(self) -> Optional[CallExpression]:
        ...
        if (
            (left := self.expr())
            and (self.expect("("))
            and (right_a := self.expr())
            and (right_b := self._loop0_8(),)
            and (self.expect(","),)
            and (self.expect(")"))
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
    ...
    def type_expr(self) -> Optional[TypeExpr]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if data_type_expr := self.data_type_expr():
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return TypeExpr(
                data_type_expr,
        ...
        self._reset(mark)
        if func_type_expr := self.func_type_expr():
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return TypeExpr(
                func_type_expr,
    ...
    def data_type_expr(self) -> Optional[PrimitiveTypeExpr]:
        ...
        if (
            (name := self.name())
            and (sizetype_expr_list := self.sizetype_expr_list(),)
            and (type_expr_list := self.type_expr_list(),)
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
    ...
    def func_type_expr(self) -> Optional[FuncTypeExpr]:
        ...
        if (
            ...
            and (type_expr_list := self.type_expr_list(),)
            and (self.expect("("))
            and (arg_type_a := self.type_expr())
            and (arg_type_b := self._loop0_9(),)
            and (self.expect(","),)
            and (self.expect(")"))
            and (self.expect(":"))
            and (return_type := self.type_expr())
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return FuncTypeExpr(
    ...
    def type_expr_list(self) -> Optional[Any]:
        ...
        if (
            (self.expect("<"))
            and (type_expr_a := self.type_expr())
            and (type_expr_b := self._loop0_10(),)
            and (self.expect(","),)
            and (self.expect(">"))
        ):
    ...
    def sizetype_expr(self) -> Optional[Any]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (self.expect("...")) and (name := self.name()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return SizeTypeExpr(
                SpreadSizeType(
    ...
    def sizetype_expr_list(self) -> Optional[Any]:
        ...
        if (
            (self.expect("["))
            and (sizetype_expr_a := self.sizetype_expr())
            and (sizetype_expr_b := self._loop0_11(),)
            and (self.expect(","),)
            and (self.expect("]"))
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
    ...
    def _loop1_2(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while _newline := self.expect("NEWLINE"):
    ...
    def _tmp_12(self) -> Optional[Any]:
        # _tmp_12: ',' import_item
        mark = self._mark()
        if (literal := self.expect(",")) and (import_item := self.import_item()):
    ...
    def _tmp_13(self) -> Optional[Any]:
        # _tmp_13: ',' arg_decl
        mark = self._mark()
        if (literal := self.expect(",")) and (arg_decl := self.arg_decl()):
    ...
    def _tmp_15(self) -> Optional[Any]:
        # _tmp_15: ',' type_expr
        mark = self._mark()
        if (literal := self.expect(",")) and (type_expr := self.type_expr()):
    ...
    def _tmp_17(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (literal := self.expect(",")) and (sizetype_expr := self.sizetype_expr()):
